...
```{r}
toJSON(BondFunds, dataframe = "rows")
```

### Columns

```{r}
toJSON(BondFunds, dataframe = "columns")
```

### Values

```{r}
toJSON(BondFunds, dataframe = "values")
```
...
1. A spreadsheet is a visualization.   
2. In some ways, so is JSON.  
3. Two types of summary  
4. Two dimensions: center and spread.  
5. Pivots and formulae
6. Visualizations and types
...
1. Reading: Chapters 1 and 2 and Wickham on Data Visualization
2. Deliverable: An In-class exercise [Assignment 5] if not completed before departing
