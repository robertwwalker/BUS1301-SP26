...
* P(Lived|Inoculated): 0.9754.  
* P(Died|Not Inoculated): 0.8589.  
* P(Lived|Not Inoculated): 0.1411.  

The computation on the right is the law of total probability.
:::

::::

* P(Died,Inoculated): 0.0392*0.0246=0.00096.  
* P(Lived,Inoculated): 0.0392*0.9754=0.03824.  
* P(Died,Not Inoculated): 0.9608*0.8589=0.82523.  
* P(Lived,Not Inoculated): 0.9608*0.1411=0.13557.  
...
The trick is (1) that evidence has to match or charges would be dropped and (2) we cannot escape the `prior`.

[A citation](https://www.tandfonline.com/doi/abs/10.1080/10691898.2014.11889692)
...
* Blood type: P(NG|Match) = 0.45.  
* Fingerprints: P(NG|Match) = 0.21.  
* DNA: P(NG|Match) = 0.017.     

$$ P(G|M,M,M) = \frac{P(G)*1*1*1}{P(G)*1*1*1 + (1-P(G))*0.45*0.21*0.017} $$

## The Bayes Factor: A Ratio

$$ P(G|M,M,M) = \frac{P(G)*1*1*1}{P(G)*1*1*1 + (1-P(G))*0.45*0.21*0.017} \\ P(NG|M,M,M) = \frac{(1-P(G))*0.45*0.21*0.017}{P(G)*1*1*1 + (1-P(G))*0.45*0.21*0.017} $$


## A Core Idea: Independence {background-image="./img/BGJury.png" background-size="contain"}
...
**A systematic description of a phenomenon that shares important and essential features of that phenomenon.**

Worth reminding,

1. All models are wrong, some are useful.  George E. P. Box.  
2. Any consistent formal mathematical system complex enough to include basic arithmetic is inherently incomplete.  Kurt G&ouml;del.


## Our Applications {background-image="./img/AppPic.png"}

[A Shinylive](https://robertwwalker.github.io/GeminiStatsDist/)
