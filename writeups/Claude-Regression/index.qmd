...
- **95% CI band** --- confidence in the *mean* cost at a given volume level,
  narrowest near $ar{x}$ and widening at the extremes.
...
$$\hat{y} \pm t_{n-2,\,0.025} \cdot \hat{\sigma}\sqrt{1 + rac{1}{n} + rac{(x^* - ar{x})^2}{\sum(x_i - ar{x})^2}}$$
...
|:---|---:|---:|
| Intercept $eta_0$ | \$33,500 | Varies by sample |
| Slope $eta_1$ | \$150 / unit | Varies by sample |
| Error std dev $\sigma$ | \$4,000 | RMSE $pprox$ \$4,000 |

: Parameter truth vs. estimation {.striped .hover}

::: {.callout-warning}
## Sampling variability
Resample repeatedly to observe how much $\hat{eta}_0$, $\hat{eta}_1$, and
